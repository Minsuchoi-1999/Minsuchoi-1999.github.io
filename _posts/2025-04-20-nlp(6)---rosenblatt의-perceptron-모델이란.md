...
하지만 퍼셉트론은 두 가지의 큰 한계를 지녔다.

1. 단층 퍼셉트론은 선형 분리만 가능하다.

2. 학습 방식이 자극과 정답에 대한 강화·억제 규칙만으로 구성되어 있어, 복잡한 패턴을 유연하게 학습하는 데 한계가 있었다.

조금 더 자세하게 설명하자면 왼쪽 그림은 직선을 통해서 얼추 두 집단을 분류해내지만, 오른쪽은 직선을 통해서는 절대 분류할 수 없다.

<img src = "https://res.cloudinary.com/dtloik0ts/image/upload/v1745237358/d68f723a-9d71-479a-a812-1096489353a7.png">

왼쪽과 같은 경우 **선형 분류**가 가능하다고 하고, 오른쪽과 같은 경우 **선형 분류**가 불가능하고 **비선형 분류**가 필요하다고 한다.

우리가 배운 퍼셉트론은 각 가중치에 단순 덧셈, 뺄셈만 하기 때문에 사실 초기값 벡터와 가중치 조정 벡터가 주어지면, 두 벡터의 1차 선형 결합으로 나타나며, 이러한 한계 때문에 자체적으로 선형 분리 이상의 문제를 풀 수 없다.

이를 나타내는 상징적인 문제가 있는데, **XOR 문제**이다.

논리에서 XOR은 완전히 베타적인 경우만 참이되는데, 이를 좌표 평면상에 도식하면 다음과 같다.

<img src = "https://res.cloudinary.com/dtloik0ts/image/upload/v1745237707/78e6e911-b789-4cd2-b693-e40b0bd490a3.png">

이처럼, 어떤 직선으로도 두 클래스를 완벽히 분리할 수 없기 때문에, 단층 퍼셉트론으로는 이 문제를 해결할 수 없다.

다음 시간에는 퍼셉트론을 직접 구현해보고, 이론에서 배운 개념들이 실제로 어떻게 작동하는지를 실습을 통해 확인해보겠다.

이후에는 이 한계를 어떻게 극복했는지, 신경망의 발전을 함께 살펴볼 예정이다.